# Adaptation to Overleaf

The following changes are made to adapt to Overleaf:

- Moved main file (`kactl.tex`) to root directory
- Added `.latexmkrc` file to specify `$ENV{'TEXINPUTS'}` according to `Makefile`
- In `content/tex/kactlpkg.sty`, modified the value of `@builddir` from `build` to `.` (current directory)
- In `content/tex/preprocessor.py`, commented out the logic of calculating the hash and put a placeholder `[HASH]` instead (line 149 to 154)

`Makefile` is also modified according to the above modifications, so that you can compile locally on linux :)

In `content/tex/preprocessor.py` we also set `Description` to be not necessary
